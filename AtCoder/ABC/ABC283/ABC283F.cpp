...
int main() {
    ...
    segtree<int, opMin, eMin> big1(n + 1), smol1(n + 1);
    for (int i = 1; i <= n; ++i) {
        ans[i] = min({ans[i], i + p[i] + smol1.prod(0, p[i]), i - p[i] + big1.prod(p[i], n + 1)});
        smol1.set(p[i], - p[i] - i);
        big1.set(p[i], p[i] - i);
    }
    segtree<int, opMin, eMin> big2(n + 1), smol2(n + 1);
    for (int i = n; i >= 1; --i) {
        ans[i] = min({ans[i], - i + p[i] + smol2.prod(0, p[i]), - i - p[i] + big2.prod(p[i], n + 1)});
        smol2.set(p[i], - p[i] + i);
        big2.set(p[i], p[i] + i);
    }
    for (int i = 1; i <= n; ++i)
