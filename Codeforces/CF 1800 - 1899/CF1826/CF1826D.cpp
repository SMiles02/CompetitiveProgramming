...
void solve() {
    ...
    vector<array<int, 3>> v = {{-1, 0, (int)1e9}};
    cin >> a[0];
    seg.set(0, a[0]);
    for (int i = 1; i < n; ++i) {
        cin >> a[i];
        if (i > 1)
            ans = max(ans, seg.prod(0, i - 1) + a[i] - i);
        seg.set(i, a[i] + i);
        while (v.back()[2] <= a[i]) {
            seg.apply(v.back()[0], v.back()[1], -v.back()[2]);
            v.pop_back();
        }
        seg.apply(v.back()[1], i, a[i]);
        v.push_back({v.back()[1], i, a[i]});
    }
    cout << ans << "\n";
}
