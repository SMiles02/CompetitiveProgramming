...
// basic operation 
S op(S l, S r) { return add(l, r); }

// op(anything, e()) = anything
S e() { return 0; }

// how a lazy update affects a value
S mapping(F l, S r) { return add(l, r); }
 
// how lazy update l affects existing lazy r
F composition(F l, F r) { return add(l, r); }
...
void solve() {
    int n, l = 1;
    cin >> n;
    segtree<int, max_op, max_e> seg_max(n + 1);
    ...
    for (int i = 1; i <= n; ++i) {
        if (a[i] < a[i - 1])
            l = i;
        mx = a[i];
        dp.apply(i, seg_max.max_right<ok>(i), dp.prod(l - 1, i));
    }
    cout << dp.get(n) << "\n";
}
